...
#[test]
fn api_post_with_input_file_sends_json_body() {
    let server = MockServer::start();
    ...
    write_config(&config_path, &format!("{}/2.0", server.base_url()));
    fs::write(&body_path, r#"{"content":{"raw":"reply body"}}"#).unwrap();
    ...
    let output = bb_command()
        .args([
            ...
            "POST",
            "--input",
            body_path.to_str().expect("path should be utf-8"),
...
#[test]
fn api_rejects_paginate_with_input() {
    let temp = tempdir().unwrap();
    let config_path = temp.path().join("config.json");
    let body_path = temp.path().join("body.json");
    write_config(&config_path, "https://api.bitbucket.org/2.0");
    fs::write(&body_path, r#"{"content":{"raw":"reply body"}}"#).unwrap();
...
#[test]
fn api_invalid_input_json_emits_error() {
    let server = MockServer::start();
    let temp = tempdir().unwrap();
    let config_path = temp.path().join("config.json");
    let body_path = temp.path().join("body.json");
...
#[test]
fn pipeline_get_build_json_resolves_uuid_from_server() {
    let server = MockServer::start();
    let lookup = server.mock(|when, then| {
        when.method(GET)
            .path("/2.0/repositories/acme/widgets/pipelines/17");
        then.json_body(json!({
            "uuid": "{pipe-123}",
            "build_number": 17
        }));
...
#[test]
fn pipeline_get_build_rejects_mismatched_lookup_result() {
    let server = MockServer::start();
    let lookup = server.mock(|when, then| {
        when.method(GET)
            .path("/2.0/repositories/acme/widgets/pipelines/17");
    ...
    let pipeline = server.mock(|when, then| {
        when.method(GET).path(
            "/2.0/repositories/acme/widgets/pipelines/%7Baaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa%7D",
        );
        then.json_body(json!({
            "uuid": "{aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa}",
            "build_number": 1,
    ...
    assert_eq!(body["error"]["code"], "internal_error");
    lookup.assert();
    assert_eq!(pipeline.hits(), 0);
}

#[test]
fn pipeline_get_build_not_found_emits_api_error() {
    let server = MockServer::start();
    let lookup = server.mock(|when, then| {
        when.method(GET)
            .path("/2.0/repositories/acme/widgets/pipelines/17");
        then.status(404).json_body(json!({
            "error": {
                "message": "Not found",
                "detail": "Pipeline with build number '17' not found in repository acme/widgets."
            }
        }));
    });
...
#[test]
fn pipeline_steps_build_json_resolves_uuid_from_server() {
    let server = MockServer::start();
    let lookup = server.mock(|when, then| {
        when.method(GET)
            .path("/2.0/repositories/acme/widgets/pipelines/17");
...
#[test]
fn pipeline_log_build_text_resolves_uuid_from_server() {
    let server = MockServer::start();
    let lookup = server.mock(|when, then| {
        when.method(GET)
            .path("/2.0/repositories/acme/widgets/pipelines/17");
...
fn pipeline_get_positional_build_resolves_uuid_from_server() {
    ...
    let lookup = server.mock(|when, then| {
        when.method(GET)
            .path("/2.0/repositories/acme/widgets/pipelines/14588");
        then.json_body(json!({
            "uuid": "{pipe-123}",
            "build_number": 14588
        }));
...
#[test]
fn pr_comment_help_includes_parent_flag() {
    let output = bb_command()
...
#[test]
fn pr_comment_with_parent_sends_reply_body() {
    let server = MockServer::start();
    let comment = server.mock(|when, then| {
        when.method(POST)
            .path("/2.0/repositories/acme/widgets/pullrequests/42/comments")
        ...
        then.json_body(json!({
            ...
            "links": {
                "html": {
                    "href": "https://bitbucket.example/pr/42/comments/9"
                }
            }
        }));
    });
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8(output.stdout).expect("stdout should be utf-8");
    assert!(stdout.contains("Created comment #9 on PR #42"));
    comment.assert();
}
